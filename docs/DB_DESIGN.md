...
| milk_type  | Type of milk            | VARCHAR(20) | Must be one of: `almond`, `coconut`, `oat`, and `soy`                               |
...
| name        | District name      | VARCHAR(30)   | NOT NULL, Must be one of: 'Moka', 'Port Louis',         |
...
The trigger named "UpdateStockLevel" is set to execute after each insertion operation on the order_product table. It functions to automatically adjust the stock level of products following an order placement. Specifically, it decreases the stock level of the products included in the order based on the quantity ordered. There are no additional constraints or conditions applied to this trigger.